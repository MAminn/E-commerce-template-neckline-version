...
export const productReview = pgTable("product_review", {
   ...
   *
   * The column default is "approved" ON PURPOSE. Reviews written before
   * moderation existed are already publicly visible, and a "pending" default
   * would retroactively hide every one of them the moment this migration
   * runs. New submissions are therefore set to "pending" EXPLICITLY by
   * create-review rather than relying on this default.
   */
  status: text("status").notNull().default("approved"),
  /** Optional customer media (image or video) shown on the review card. */
...
/** Allowed values for `productReview.status`. */
