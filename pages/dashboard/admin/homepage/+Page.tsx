...
/**
 * Icon choices for the Brand Statement feature items. Every ValuePropIconType
 * is offered — the section is a free-form feature list, so restricting the set
 * would only stop merchants describing their own product.
 */
const BRAND_STATEMENT_ICON_OPTIONS: { value: ValuePropIconType; label: string }[] =
  [
    { value: ValuePropIconType.BRIEFCASE, label: "Portable (Briefcase)" },
    { value: ValuePropIconType.HEART, label: "Loved (Heart)" },
    { value: ValuePropIconType.BADGE, label: "Verified (Badge check)" },
    { value: ValuePropIconType.HEAT, label: "Heat / Diffusion (Waves)" },
    { value: ValuePropIconType.BAN, label: "Free from (Crossed circle)" },
    { value: ValuePropIconType.GLOBE, label: "Planet (Globe)" },
    { value: ValuePropIconType.CLEAN, label: "Clean (Leaf)" },
    { value: ValuePropIconType.POCKET, label: "Pocket (Hand)" },
    { value: ValuePropIconType.CLOCK, label: "Time (Clock)" },
    { value: ValuePropIconType.PACKAGE, label: "Package (Box)" },
    { value: ValuePropIconType.BOTTLE, label: "Bottle (Flask)" },
    { value: ValuePropIconType.QUALITY, label: "Quality (Award)" },
    { value: ValuePropIconType.SECURITY, label: "Security (Shield)" },
    { value: ValuePropIconType.SHIPPING, label: "Shipping (Truck)" },
    { value: ValuePropIconType.SHOPPING, label: "Shopping (Bag)" },
    { value: ValuePropIconType.SUPPORT, label: "Support (Headphones)" },
    { value: ValuePropIconType.RETURNS, label: "Returns (Refresh)" },
    { value: ValuePropIconType.RECEIPT, label: "Receipt" },
    { value: ValuePropIconType.PAYMENT, label: "Payment (Card)" },
  ];
...
export default function HomepageAdminPage() {
  ...
  /**
   * Upload media for one testimonial. Mirrors the brand-statement uploader
   * and reuses the same generic endpoint, with `preserveAspect` so a review
   * photo is not centre-cropped to hero proportions.
   */
  const handleUploadTestimonialMedia = async (
    event: React.ChangeEvent<HTMLInputElement>,
    index: number,
  ) => {
    const file = event.target.files?.[0];
      ...
      <div className='space-y-6'>
        ...
        {!isMinimal && (
          <Card>
            ...
            <CardContent className='space-y-4'>
              <div>
                <Label htmlFor='brand-statement-eyebrow'>
                  Eyebrow / small label (optional)
                </Label>
                <Input
                  id='brand-statement-eyebrow'
                  ...
                  placeholder='Why solid is better'
                  disabled={!content.brandStatement.enabled}
                />
                <p className='text-sm text-muted-foreground mt-1'>
                  Shown above the title in the accent colour. Leave empty to
                  show a short accent rule instead.
                </p>
              </div>
              ...
              {/* Feature items rendered around the statement image. The first
                  half fills the left column, the second half the right, so an
                  even count (6 is the reference) splits 3/3. Templates that do
                  not render these simply ignore them. */}
              <div className='border-t pt-4'>
                <div className='flex items-center justify-between mb-1'>
                  <Label>Feature items</Label>
                  <span className='text-sm text-muted-foreground'>
                    {(content.brandStatement.benefits ?? []).length}/6
                  </span>
                </div>
                <p className='text-sm text-muted-foreground mb-3'>
                  Arranged around the image — the first half on the left, the
                  rest on the right. Line breaks in a description are kept, so
                  you can control exactly where each line wraps.
                </p>

                <div className='space-y-4'>
                  {(content.brandStatement.benefits ?? []).map(
                    (benefit, index) => (
                      <div
                        ...
                        <div>
                          ...
                          <Select
                            ...
                            <SelectContent>
                              {BRAND_STATEMENT_ICON_OPTIONS.map((opt) => (
                        ...
                        <div>
                          ...
                          <Input
                            ...
                            placeholder='Perfectly portable'
                            disabled={!content.brandStatement.enabled}
                          />
                        </div>
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value={ValuePropIconType.CLEAN}>
                          Clean (Leaf)
                        </SelectItem>
                        <SelectItem value={ValuePropIconType.POCKET}>
                          Pocket (Hand)
                        </SelectItem>
                        <SelectItem value={ValuePropIconType.CLOCK}>
                          Time (Clock)
                        </SelectItem>
                      </SelectContent>
                    </Select>
            ...
            <div className='space-y-4'>
              {(content.testimonials?.items ?? []).map((item, idx) => (
                <div
                  ...
                  {/* Media + product label — consumed by the Noir review
                      wall. Other templates ignore these fields. */}
                  <div className='grid grid-cols-2 gap-3'>
                    <div>
                      <Label className='text-xs'>
                        Media URL (image or video, optional)
                      </Label>
                      <Input
                        value={item.mediaUrl ?? ""}
                        onChange={(e) =>
                        ...
                        placeholder='/uploads/review-1.jpg'
                        disabled={!(content.testimonials?.enabled ?? false)}
                      />
                      <div className='flex items-center gap-2 mt-2'>
                        <input
                          type='file'
                          id={`testimonial-media-${idx}`}
                          accept='image/jpeg,image/jpg,image/png,image/webp'
                          className='hidden'
                          disabled={
                          ...
                          onChange={(e) =>
                            handleUploadTestimonialMedia(e, idx)
                          }
                        />
                        <Button
                          type='button'
                          ...
                          onClick={() =>
                            document
                              .getElementById(`testimonial-media-${idx}`)
                              ?.click()
                          }>
                          {isUploadingImage ? "Uploading..." : "Upload Image"}
                        </Button>
                              ...
                              setContent((prev) => {
                                ...
                                return {
                                  ...
                                  testimonials: {
                                    ...prev.testimonials!,
                                    enabled:
                                      prev.testimonials?.enabled ?? false,
                                    items,
                                  },
                                };
                              })
                      ...
                      <p className='text-xs text-muted-foreground mt-1'>
                        Upload handles images. For video, paste a URL to an
                        .mp4 / .webm file.
                      </p>
                    </div>
                    ...
                    <div className='space-y-3'>
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setContent((prev) => {
                              ...
                              items[idx] = {
                                ...items[idx],
                                productLabel: e.target.value || undefined,
                              } as (typeof items)[number];
                              return {
                                ...prev,
                                testimonials: {
                      ...
                      <div className='flex items-center gap-2'>
                        ...
                        />
                        <Label
                          htmlFor={`testimonial-verified-${idx}`}
                          className='text-xs'>
                          Verified reviewer
                        </Label>
                      </div>
