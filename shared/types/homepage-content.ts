...
export enum ValuePropIconType {
  ...
  CLEAN = "clean",
  /** Open palm — pocket-friendly / compact / carry-anywhere. */
  POCKET = "pocket",
  /** Clock — longevity / long lasting. */
  CLOCK = "clock",
  /** Briefcase — portability / travel-friendly. */
  BRIEFCASE = "briefcase",
  /** Heart — desirability / loved by customers. */
  HEART = "heart",
  /** Seal with a check — verified quality / standout. */
  BADGE = "badge",
  /** Rising waves — warmth, diffusion, body-heat activation. */
  HEAT = "heat",
  /** Crossed circle — free from / contains no X. */
  BAN = "ban",
  /** Globe — natural, recyclable, planet-friendly. */
  GLOBE = "globe",
}
...
export interface HomepageContent {
  ...
  testimonials?: {
    ...
    title?: string;
    titleAr?: string;
    /** Supporting line under the title. Consumed by Noir; others ignore it. */
    subtitle?: string;
    subtitleAr?: string;
    /**
     * Optional overrides for the rating summary row. Left unset — the normal
     * case — the row shows the REAL average and count of approved reviews.
     * They exist for stores that aggregate ratings off-site (a review
     * platform) and need the headline figure to match that source.
     */
    ratingOverride?: number;
    reviewCountOverride?: number;
    /** Bottom CTA. Noir falls back to "Read All Reviews" → /shop. */
    ctaText?: string;
    ctaLink?: string;
    items: {
      name: string;
      ...
      reviewAr?: string;
      /**
       * Optional photo or video for the review card. Additive: templates
       * that do not render media simply ignore it, and a testimonial without
       * media still renders as a quote-only card.
       */
      mediaUrl?: string;
      /**
       * Overrides how `mediaUrl` is rendered. Left unset, the type is
       * inferred from the file extension — this exists for CDN or signed
       * URLs that carry no extension to infer from.
       */
      mediaType?: "image" | "video";
      /** Scent / product the review is about, shown under the name. */
      productLabel?: string;
      productLabelAr?: string;
      /** Marks this reviewer as verified. Off by default. */
      verified?: boolean;
    }[];
  };
}
...
export const DEFAULT_HOMEPAGE_CONTENT: HomepageContent = {
  ...
  brandStatement: {
    enabled: true,
    // Newline is deliberate: the Noir section renders the title with
    // `whitespace-pre-line`, so this is the reference two-line break. Other
    // templates collapse it to a space, as they always did.
    title: "Smarter scents.\nBetter for you.",
    ...
    /**
     * Six items: the first three render in the left column, the last three in
     * the right. Newlines in `description` are preserved by the Noir section,
     * which is how the reference gets its deliberate line breaks — merchants
     * can re-break the copy without touching code.
     */
    benefits: [
      {
        icon: ValuePropIconType.BRIEFCASE,
        title: "Perfectly portable",
        description: "Compact and TSA-friendly.\nTake your scent anywhere.",
      },
      {
        icon: ValuePropIconType.HEART,
        title: "Compliment optimized",
        description:
          "Subtle, skin-close scent\nthat gets noticed for\nall the right reasons.",
      },
      {
      ...
      {
        icon: ValuePropIconType.HEAT,
        title: "Body-heat activated",
        description:
          "Melts into your skin\nand releases fragrance\nthroughout the day.",
      },
      {
        icon: ValuePropIconType.BAN,
        title: "Paraben-free formula",
        description:
          "Clean, non-toxic, and\nsafe for everyday\nuse on your skin.",
      },
      {
        icon: ValuePropIconType.GLOBE,
